...
function service_ip {
	...
	for _ in $(seq 1 5); do
		...
		debug="$(docker container inspect "$cid" --format '{{ (index .NetworkSettings.Networks)}}')"
		echo "Debug: $debug" >&2
...
function poll_ready {
	...
	local -a args=( '-v' '-D-' '-m3' '-w' '%{http_code}' "$url" )
	if [ "$#" -ge 3 ]; then
		args+=( ${@:3} )
	fi
	...
	for _ in $(seq 1 60); do
		echo 'Container Status '$url': ' $(docker container inspect "$cid" --format '{{ .State.Status}}') >&1
